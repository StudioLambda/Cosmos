...
`framework.Handler` returns an error. Errors implementing `framework.HTTPStatusError` select a status; errors implementing `http.Handler`, including `problem.Details`, render themselves. A handler that neither writes a response nor returns an error receives `204 No Content`.
